        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 max-w-7xl mx-auto">
          {loading ? (
            ...
            Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="bg-gradient-to-br from-gray-900 to-gray-800 p-6 rounded-2xl border border-gray-700 animate-pulse min-h-[340px]"
              />
            ))
          ) : (
            jobs.map((job, index) => (
              <motion.div
                ...
                className="bg-gradient-to-br from-gray-900 to-gray-800 p-6 rounded-2xl border border-gray-700 hover:border-blue-500 transition-all duration-300 group cursor-pointer transform-gpu will-change-transform min-h-[340px] flex flex-col w-full"
              >
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {job.labelStack &&
                      job.labelStack
                        ...
                        .filter(tech => tech.trim().length > 0) // Remove strings vazias
                        .slice(0, 3)
                        .map((tech, index) => (
                          <motion.span
                            key={`${tech}-${index}`}
                    ...
                    {job.labelStack &&
                      job.labelStack.split(/\s+/).filter(tech => tech.trim().length > 0).length > 3 && (
                ...
                <motion.button
                  ...
                  className="w-full py-3 bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg text-white font-semibold hover:from-blue-700 hover:to-purple-700 transition-all duration-300 mt-auto"
                >
